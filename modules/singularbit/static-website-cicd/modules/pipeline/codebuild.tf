resource "aws_codebuild_project" "codebuild_deployment" {
  ...
  for_each   = var.static_website_pipeline_build_stages

  name         = "${var.project_name}-${var.static_website_key}-${each.key}"
  description  = "CodeBuild Role for ${var.project_name}-${var.static_website_key}-${each.key}"
...
#    image                       = "aws/codebuild/amazonlinux2-x86_64-standard:3.0"
  ...
  logs_config {
    cloudwatch_logs {
      group_name = "${var.project_name}-${var.static_website_key}-${each.key}-log-group"
      stream_name = "${var.project_name}-${var.static_website_key}-${each.key}log-stream"
    }

    s3_logs {
      status = "ENABLED"
      location = "${var.s3_id_codebuild_logs}/${each.key}/build_logs"
    }
  }
